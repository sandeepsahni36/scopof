...
const TemplateDetailPage = () => {
  ...
  // Helper function to find insertion index after a section and its children
  const findInsertIndexAfterSectionInArray = (sectionIndex: number, currentItems: FormValues['items']): number => {
    const sectionId = currentItems[sectionIndex]?.id;
    if (!sectionId) return currentItems.length;
    
    let insertIndex = sectionIndex + 1;
    
    // Find the end of this section's children
    for (let i = sectionIndex + 1; i < currentItems.length; i++) {
      if (currentItems[i].parentId === sectionId) {
        insertIndex = i + 1;
      } else {
        break;
      }
