...
import {
  Plus, Trash2, GripVertical, ArrowLeft,
  Type, CheckSquare, Square, Camera, Hash, Minus
} from 'lucide-react';
import { Button } from '../../components/ui/Button';
import { Input } from '../../components/ui/Input';
import {
  TemplateItemType, RatingOption, RATING_COLORS, TemplateCategory
} from '../../types';
import {
  getTemplate, createTemplate, updateTemplate, getTemplateCategories
} from '../../lib/templates';
...
const TemplateDetailPage = () => {
  ...
  const {
    ...
    defaultValues: { name: '', description: '', categoryId: '', items: [] },
  });
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      if (isNew) {
        const ok = await createTemplate(templateData, apiItems);
        if (ok) {
          toast.success('Template created successfully');
          navigate('/dashboard/templates');
        }
      } else {
        const ok = await updateTemplate(id!, templateData, apiItems);
        if (ok) {
  ...
  const addOptionToItem = (itemIndex: number) => {
    const current = (watch(`items.${itemIndex}.options`) as RatingOption[]) || [];
    setValue(`items.${itemIndex}.options`, [...current, { label: '', color: RATING_COLORS.green }]);
  };
  const removeOptionFromItem = (itemIndex: number, optionIndex: number) => {
    const current = (watch(`items.${itemIndex}.options`) as RatingOption[]) || [];
    setValue(`items.${itemIndex}.options`, current.filter((_, i) => i !== optionIndex));
  };
  const updateOptionLabel = (itemIndex: number, optionIndex: number, label: string) => {
    const current = (watch(`items.${itemIndex}.options`) as RatingOption[]) || [];
    const next = [...current]; next[optionIndex] = { ...next[optionIndex], label };
    setValue(`items.${itemIndex}.options`, next);
  };
  const updateOptionColor = (itemIndex: number, optionIndex: number, color: string) => {
    const current = (watch(`items.${itemIndex}.options`) as RatingOption[]) || [];
    const next = [...current]; next[optionIndex] = { ...next[optionIndex], color };
    setValue(`items.${itemIndex}.options`, next);
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white shadow rounded-lg p-6 sticky top-6">
              ...
              {/* Make buttons “boxed” and readable on mobile */}
              <div className="space-y-3">
                <Button
                  type="button" variant="outline" size="sm"
                  fullWidth
                  className="justify-start border-2 border-primary-300 text-primary-700 hover:bg-primary-50 font-medium"
                ...
                <Droppable droppableId="template-items">
                  {(provided, snapshot) => (
                    <div
                      ...
                      className={`min-h-[260px] rounded-lg border-2 border-dashed transition-colors p-4
                          ...
                          {(p, s) => (
                            <div
                              ...
                              className={`border border-gray-200 rounded-lg bg-white transition-all duration-200 cursor-pointer
                                ${s.isDragging ? 'shadow-lg rotate-1 scale-105' : 'shadow-sm hover:shadow-md'}
                ...
                <div className="space-y-4">
                  ...
                  {(selectedItem.type === 'single_choice' || selectedItem.type === 'multiple_choice') && (
                    <div>
                      ...
                      <div className="space-y-3">
                        {((watch(`items.${selectedItemIndex}.options`) as RatingOption[]) || []).map((option, i) => (
                          <div key={i} className="flex items-center space-x-2">
                            <input
                              type="text"
                              value={option.label}
                              onChange={(e) => updateOptionLabel(selectedItemIndex!, i, e.target.value)}
                            ...
                            <div className="flex space-x-1">
                              {Object.entries(RATING_COLORS).map(([k, v]) => (
                                <button
                                  key={k}
                                  type="button"
                                  onClick={() => updateOptionColor(selectedItemIndex!, i, v)}
                                  className={`w-6 h-6 rounded border-2 transition-all ${
                                    option.color === v ? 'border-gray-800 scale-110' : 'border-gray-300 hover:border-gray-400'
                                  }`}
                            ...
                            <Button type="button" variant="ghost" size="sm" onClick={() => removeOptionFromItem(selectedItemIndex!, i)} leftIcon={<Trash2 size={16} />} />
                          </div>
                        ))}
