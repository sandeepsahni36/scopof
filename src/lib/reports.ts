...
const adminNavItems: NavItem[] = [
  ...
  { title: 'Company', href: '/dashboard/admin/company', icon: 'Building2' }
];
...
const DashboardLayout: React.FC = () => {
  ...
  const isTrialExpired = company?.subscription_status === 'trial_expired';
  ...
  const showTrialWarning = isTrialing && trialDaysRemaining <= 7;
  ...
  async function generateInspectionReport(reportData: any): Promise<string> {
    try {
      ...
      if (devModeEnabled()) {
        ...
        reportData = generateMockPDFReport(reportData);
      }
      ...
      const uploadResult = await uploadFileWithPropertyName(
        ...
        reportData.inspection.propertyName || 'property',
        reportData
      );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <motion.aside
        ...
        className="fixed inset-y-0 left-0 z-50 bg-white shadow-lg border-r border-gray-200 hidden md:flex flex-col"
      >
