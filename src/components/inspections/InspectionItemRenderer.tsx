...
interface InspectionItemRendererProps {
  item: MutableInspectionItem;
  inspectionId: string;
  onUpdate: (itemId: string, updates: Partial<MutableInspectionItem>) => void;
}
...
>(({
  ...
  const [photos, setPhotos] = useState<string[]>(item.photoUrls);
  const [markedForReport, setMarkedForReport] = useState(item.markedForReport);
  const [reportRecipientId, setReportRecipientId] = useState(item.reportRecipientId);
  ...
  // Update local state when item prop changes (e.g., when navigating back to a step)
  useEffect(() => {
    setValue(item.value);
    setNotes(item.notes);
    setPhotos(item.photoUrls);
    setMarkedForReport(item.markedForReport);
    setReportRecipientId(item.reportRecipientId);
  }, [item]);
  useEffect(() => {
  ...
  const handleMarkedForReportChange = async (checked: boolean) => {
    ...
    const updates: Partial<MutableInspectionItem> = { markedForReport: checked };
    if (!checked) {
      setReportRecipientId('');
      updates.reportRecipientId = '';
    }
    // Immediately notify parent of the change
  ...
  const handleReportRecipientChange = async (recipientId: string) => {
    ...
    // Immediately notify parent of the change
    onUpdate(item.id, { reportRecipientId: recipientId });
  };
  ...
  const handlePhotoUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (photoUrl) {
        ...
        setPhotos(newPhotos);
        // Immediately notify parent of the change
        onUpdate(item.id, { photoUrls: newPhotos });
