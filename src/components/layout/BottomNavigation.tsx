...
const BottomNavigation = () => {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-gray-200 shadow-lg md:hidden">
      <nav className="flex justify-around items-center h-20 px-2 bg-white">
        {allNavItems.map((item) => (
          <NavLink
            key={item.href}
            to={item.href}
            className={({ isActive }) => 
              `flex flex-col items-center justify-center text-xs font-medium transition-colors min-w-0 flex-1 py-3 px-2 rounded-lg mx-1 ${
                isActive
          ...
          >
            <div className="mb-1 flex-shrink-0 p-1">
              {IconMap[item.icon]}
            </div>
            <span className="truncate text-center leading-tight text-xs font-medium">{item.title}</span>
          </NavLink>
        ))}
      </nav>
    </div>
