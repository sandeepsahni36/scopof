...
serve(async (req) => {
  ...
  try {
    ...
    console.log("Auth: Is service role request:", isServiceRoleRequest);
    
    let userContext: any = null;
    
    if (isServiceRoleRequest) {
      console.log("Auth: Service role request detected, skipping user authentication");
      // For service role requests, we don't need user context for download operations
      userContext = {
        isServiceRole: true,
        userId: null,
        adminId: null,
    ...
    switch (endpoint) {
      case "upload": {
        ...
        // Service role requests are not allowed for upload operations
        if (isServiceRoleRequest) {
          console.error("Upload: Service role requests not allowed for upload operations");
          return new Response(JSON.stringify({
            error: "Forbidden: Service role cannot upload files"
          }), {
            status: 403,
            headers: {
              ...corsHeaders,
      ...
      case "delete": {
        ...
        const fileKey = cleanedSegments.slice(1).join('/'); // Reconstruct fileKey from cleaned segments (skip 'delete')
        if (!fileKey) {
          return new Response(JSON.stringify({
            error: "File key not provided"
          }), {
        ...
        // Service role requests are not allowed for delete operations
        if (isServiceRoleRequest) {
          console.error("Delete: Service role requests not allowed for delete operations");
          return new Response(JSON.stringify({
            error: "Forbidden: Service role cannot delete files"
          }), {
            status: 403,
            headers: {
              ...corsHeaders,
      ...
      case "usage": {
        ...
        // Service role requests are not allowed for usage operations
        if (isServiceRoleRequest) {
          console.error("Usage: Service role requests not allowed for usage operations");
          return new Response(JSON.stringify({
            error: "Forbidden: Service role cannot access usage data"
          }), {
            status: 403,
            headers: {
              ...corsHeaders,
