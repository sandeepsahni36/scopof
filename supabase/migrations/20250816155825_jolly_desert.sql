```diff
--- a/supabase/migrations/20250816093440_restless_rice.sql
+++ b/supabase/migrations/20250816093440_restless_rice.sql
@@ -1,45 +1,65 @@
...
--- Indexes
-CREATE INDEX IF NOT EXISTS idx_invitations_admin_id ON public.invitations USING btree (admin_id);
...
-CREATE INDEX IF NOT EXISTS idx_invitations_status ON public.invitations USING btree (status);
...
--- Policies
-CREATE POLICY IF NOT EXISTS "invitations_delete_by_admin" ON public.invitations
+-- Recreate policies
...
-CREATE POLICY IF NOT EXISTS "invitations_insert_by_admin" ON public.invitations
...
-CREATE POLICY IF NOT EXISTS "invitations_postgres_all" ON public.invitations
+CREATE POLICY "invitations_postgres_all" ON public.invitations
   FOR ALL USING (true) WITH CHECK (true);
 
-CREATE POLICY IF NOT EXISTS "invitations_select_by_token_or_admin" ON public.invitations
+CREATE POLICY "invitations_select_by_token_or_admin" ON public.invitations
   FOR SELECT USING (((status = 'pending'::text) AND (expires_at > now())) OR ((auth.uid() IS NOT NULL) AND (EXISTS ( SELECT 1
    FROM (admin a
      JOIN team_members tm ON ((a.id = tm.admin_id)))
   WHERE ((a.id = invitations.admin_id) AND (tm.profile_id = auth.uid()) AND (tm.role = ANY (ARRAY['owner'::team_member_role, 'admin'::team_member_role])))))));
 
-CREATE POLICY IF NOT EXISTS "invitations_update_by_admin_or_invitee" ON public.invitations
+CREATE POLICY "invitations_update_by_admin_or_invitee" ON public.invitations
   FOR UPDATE USING (((EXISTS ( SELECT 1
    FROM (admin a
      JOIN team_members tm ON ((a.id = tm.admin_id)))
   WHERE ((a.id = invitations.admin_id) AND (tm.profile_id = auth.uid()) AND (tm.role = ANY (ARRAY['owner'::team_member_role, 'admin'::team_member_role]))))) OR ((email = auth.email()) AND (status = 'pending'::text) AND (expires_at > now()))) WITH CHECK (((EXISTS ( SELECT 1
    FROM (admin a
      JOIN team_members tm ON ((a.id = tm.admin_id)))
   WHERE ((a.id = invitations.admin_id) AND (tm.profile_id = auth.uid()) AND (tm.role = ANY (ARRAY['owner'::team_member_role, 'admin'::team_member_role]))))) OR ((email = auth.email()) AND (status = 'pending'::text) AND (expires_at > now()))));
