...
-- Create custom types (IF NOT EXISTS added)
...
-- Create trigger functions (CREATE OR REPLACE FUNCTION used)
...
-- Authentication functions (CREATE OR REPLACE FUNCTION used)
...
-- Create tables (CREATE TABLE IF NOT EXISTS used)
...
-- Create views (CREATE OR REPLACE VIEW used)
...
-- Create indexes (CREATE INDEX IF NOT EXISTS used)
...
-- Add foreign key constraints (ALTER TABLE IF EXISTS used for safety)
ALTER TABLE IF EXISTS public.admin ADD CONSTRAINT admin_owner_id_fkey FOREIGN KEY (owner_id) REFERENCES auth.users(id);
ALTER TABLE IF EXISTS public.admin ADD CONSTRAINT admin_billing_manager_id_fkey FOREIGN KEY (billing_manager_id) REFERENCES public.profiles(id);
ALTER TABLE IF EXISTS public.profiles ADD CONSTRAINT profiles_id_fkey FOREIGN KEY (id) REFERENCES auth.users(id);
ALTER TABLE IF EXISTS public.team_members ADD CONSTRAINT team_members_admin_id_fkey FOREIGN KEY (admin_id) REFERENCES public.admin(id);
ALTER TABLE IF EXISTS public.team_members ADD CONSTRAINT team_members_profile_id_fkey FOREIGN KEY (profile_id) REFERENCES public.profiles(id);
ALTER TABLE IF EXISTS public.properties ADD CONSTRAINT properties_admin_id_fkey FOREIGN KEY (admin_id) REFERENCES public.admin(id) ON DELETE CASCADE;
ALTER TABLE IF EXISTS public.template_categories ADD CONSTRAINT template_categories_admin_id_fkey FOREIGN KEY (admin_id) REFERENCES public.admin(id);
ALTER TABLE IF EXISTS public.templates ADD CONSTRAINT templates_admin_id_fkey FOREIGN KEY (admin_id) REFERENCES public.admin(id);
ALTER TABLE IF EXISTS public.templates ADD CONSTRAINT templates_category_id_fkey FOREIGN KEY (category_id) REFERENCES public.template_categories(id);
ALTER TABLE IF EXISTS public.template_items ADD CONSTRAINT template_items_template_id_fkey FOREIGN KEY (template_id) REFERENCES public.templates(id) ON DELETE CASCADE;
ALTER TABLE IF EXISTS public.template_items ADD CONSTRAINT template_items_parent_id_fkey FOREIGN KEY (parent_id) REFERENCES public.template_items(id) ON DELETE CASCADE;
ALTER TABLE IF EXISTS public.template_items ADD CONSTRAINT template_items_report_recipient_id_fkey FOREIGN KEY (report_recipient_id) REFERENCES public.report_service_teams(id) ON DELETE SET NULL;
ALTER TABLE IF EXISTS public.report_service_teams ADD CONSTRAINT report_service_teams_admin_id_fkey FOREIGN KEY (admin_id) REFERENCES public.admin(id) ON DELETE CASCADE;
ALTER TABLE IF EXISTS public.property_checklists ADD CONSTRAINT property_checklists_property_id_fkey FOREIGN KEY (property_id) REFERENCES public.properties(id) ON DELETE CASCADE;
ALTER TABLE IF EXISTS public.property_checklist_templates ADD CONSTRAINT property_checklist_templates_property_checklist_id_fkey FOREIGN KEY (property_checklist_id) REFERENCES public.property_checklists(id) ON DELETE CASCADE;
ALTER TABLE IF EXISTS public.property_checklist_templates ADD CONSTRAINT property_checklist_templates_template_id_fkey FOREIGN KEY (template_id) REFERENCES public.templates(id) ON DELETE CASCADE;
...
-- Enable Row Level Security (ALTER TABLE IF EXISTS used for safety)
...
ALTER TABLE IF EXISTS public.report_service_teams ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS public.property_checklists ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS public.property_checklist_templates ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS public.inspections ENABLE ROW LEVEL SECURITY;
ALTER TABLE IF EXISTS public.inspection_items ENABLE ROW LEVEL SECURITY;
...
ALTER TABLE IF EXISTS public.stripe_orders ENABLE ROW LEVEL SECURITY;

-- Create RLS policies (CREATE POLICY IF NOT EXISTS used)
CREATE POLICY IF NOT EXISTS admin_owner_full_access ON public.admin
    FOR ALL TO authenticated
    USING (owner_id = auth.uid())
    WITH CHECK (owner_id = auth.uid());

CREATE POLICY IF NOT EXISTS admin_postgres_access ON public.admin
    FOR ALL TO postgres
    USING (true)
    WITH CHECK (true);
...
CREATE POLICY IF NOT EXISTS profiles_postgres_all ON public.profiles
    FOR ALL TO postgres
    USING (true)
    WITH CHECK (true);

CREATE POLICY IF NOT EXISTS team_members_admin_manage ON public.team_members
    FOR ALL TO authenticated
...
CREATE POLICY IF NOT EXISTS team_members_own_record ON public.team_members
    FOR ALL TO authenticated
    USING (profile_id = auth.uid())
    WITH CHECK (profile_id = auth.uid());

CREATE POLICY IF NOT EXISTS team_members_postgres_access ON public.team_members
    FOR ALL TO postgres
    USING (true)
    WITH CHECK (true);

CREATE POLICY IF NOT EXISTS properties_manage_admins ON public.properties
    FOR ALL TO authenticated
    USING (EXISTS (
...
CREATE POLICY IF NOT EXISTS properties_select_members ON public.properties
    FOR SELECT TO authenticated
    USING (EXISTS (
        SELECT 1 FROM admin a
        WHERE a.id = properties.admin_id
...
CREATE POLICY IF NOT EXISTS properties_postgres_all ON public.properties
    FOR ALL TO postgres
    USING (true)
    WITH CHECK (true);

CREATE POLICY IF NOT EXISTS templates_manage_admins ON public.templates
    FOR ALL TO authenticated
    USING (EXISTS (
        SELECT 1 FROM admin a
...
CREATE POLICY IF NOT EXISTS templates_select_members ON public.templates
    FOR SELECT TO authenticated
    USING (EXISTS (
        SELECT 1 FROM admin a
        WHERE a.id = templates.admin_id
...
CREATE POLICY IF NOT EXISTS templates_postgres_all ON public.templates
    FOR ALL TO postgres
    USING (true)
    WITH CHECK (true);

CREATE POLICY IF NOT EXISTS template_items_manage_admins ON public.template_items
    FOR ALL TO authenticated
    USING (EXISTS (
...
CREATE POLICY IF NOT EXISTS template_items_select_members ON public.template_items
    FOR SELECT TO authenticated
    USING (EXISTS (
        SELECT 1 FROM templates t
        JOIN admin a ON t.admin_id = a.id
...
CREATE POLICY IF NOT EXISTS template_items_postgres_all ON public.template_items
    FOR ALL TO postgres
    USING (true)
    WITH CHECK (true);

CREATE POLICY IF NOT EXISTS template_categories_manage_admins ON public.template_categories
    FOR ALL TO authenticated
    USING (EXISTS (
        SELECT 1 FROM admin a
...
CREATE POLICY IF NOT EXISTS template_categories_select_members ON public.template_categories
    FOR SELECT TO authenticated
    USING (EXISTS (
        SELECT 1 FROM admin a
        WHERE a.id = template_categories.admin_id
...
CREATE POLICY IF NOT EXISTS template_categories_postgres_all ON public.template_categories
    FOR ALL TO postgres
    USING (true)
    WITH CHECK (true);

CREATE POLICY IF NOT EXISTS report_service_teams_manage_admins ON public.report_service_teams
    FOR ALL TO authenticated
    USING (EXISTS (
        SELECT 1 FROM admin a
...
CREATE POLICY IF NOT EXISTS report_service_teams_select_members ON public.report_service_teams
    FOR SELECT TO authenticated
    USING (EXISTS (
        SELECT 1 FROM admin a
        WHERE a.id = report_service_teams.admin_id
...
CREATE POLICY IF NOT EXISTS report_service_teams_postgres_all ON public.report_service_teams
    FOR ALL TO postgres
    USING (true)
    WITH CHECK (true);

CREATE POLICY IF NOT EXISTS property_checklists_manage_admins ON public.property_checklists
    FOR ALL TO authenticated
    USING (EXISTS (
...
CREATE POLICY IF NOT EXISTS property_checklists_select_members ON public.property_checklists
    FOR SELECT TO authenticated
    USING (EXISTS (
        SELECT 1 FROM properties p
        JOIN admin a ON p.admin_id = a.id
...
CREATE POLICY IF NOT EXISTS property_checklists_postgres_all ON public.property_checklists
    FOR ALL TO postgres
    USING (true)
    WITH CHECK (true);

CREATE POLICY IF NOT EXISTS property_checklist_templates_access_for_members ON public.property_checklist_templates
    FOR ALL TO authenticated
    USING (EXISTS (
...
CREATE POLICY IF NOT EXISTS property_checklist_templates_postgres_all ON public.property_checklist_templates
    FOR ALL TO postgres
    USING (true)
    WITH CHECK (true);

CREATE POLICY IF NOT EXISTS inspections_access_for_members ON public.inspections
    FOR ALL TO authenticated
    USING (EXISTS (
        SELECT 1 FROM properties p
        JOIN admin a ON p.admin_id = a.id
...
CREATE POLICY IF NOT EXISTS inspections_postgres_all ON public.inspections
    FOR ALL TO postgres
    USING (true)
    WITH CHECK (true);

CREATE POLICY IF NOT EXISTS inspection_items_access_for_members ON public.inspection_items
    FOR ALL TO authenticated
    USING (EXISTS (
...
CREATE POLICY IF NOT EXISTS inspection_items_postgres_all ON public.inspection_items
    FOR ALL TO postgres
    USING (true)
    WITH CHECK (true);

CREATE POLICY IF NOT EXISTS reports_manage_admins ON public.reports
    FOR ALL TO authenticated
    USING (EXISTS (
        SELECT 1 FROM inspections i
        JOIN properties p ON i.property_id = p.id
...
CREATE POLICY IF NOT EXISTS reports_postgres_all ON public.reports
    FOR ALL TO postgres
    USING (true)
    WITH CHECK (true);

CREATE POLICY IF NOT EXISTS stripe_customers_postgres_all ON public.stripe_customers
    FOR ALL TO postgres
    USING (true)
    WITH CHECK (true);

CREATE POLICY IF NOT EXISTS stripe_subscriptions_postgres_all ON public.stripe_subscriptions
    FOR ALL TO postgres
    USING (true)
    WITH CHECK (true);

CREATE POLICY IF NOT EXISTS stripe_orders_postgres_all ON public.stripe_orders
    FOR ALL TO postgres
    USING (true)
    WITH CHECK (true);

-- Create triggers (DROP TRIGGER IF EXISTS and CREATE TRIGGER used)
...
-- Create auth triggers (DROP TRIGGER IF EXISTS and CREATE TRIGGER used)
