/*
  ...
  1. Updates
    - Modifies the handle_new_user function to automatically create demo templates
...
-- Update the handle_new_user function to include demo templates
...
BEGIN
    ...
    -- Create admin record and get the ID
    ...
    VALUES (
        ...
        'starter',
        CASE 
            WHEN NEW.raw_user_meta_data->>'registration_type' = 'no_trial' THEN 'not_started'
            ELSE 'trialing'
        END,
        CASE 
            WHEN NEW.raw_user_meta_data->>'registration_type' = 'no_trial' THEN NULL
            ELSE NOW()
        END,
        CASE 
            WHEN NEW.raw_user_meta_data->>'registration_type' = 'no_trial' THEN NULL
    ...
    -- Create demo templates for new users
    -- Insert Templates
    INSERT INTO public.templates (id, admin_id, category_id, name, description) VALUES
    (gen_random_uuid(), admin_id_var, null, 'Holiday Homes Connections and Door', 'Holiday Homes'),
    (gen_random_uuid(), admin_id_var, null, 'Living Room', 'Furnished'),
    (gen_random_uuid(), admin_id_var, null, 'Kitchen', 'Fully Equipped'),
    (gen_random_uuid(), admin_id_var, null, 'Balcony', 'Holiday Homes Furniture'),
    (gen_random_uuid(), admin_id_var, null, 'Bedroom', null),
    (gen_random_uuid(), admin_id_var, null, 'Holiday Homes General and Cleaning Items', null),
    (gen_random_uuid(), admin_id_var, null, 'Bathroom', 'Holiday Homes');

    -- Get the template IDs that were just inserted
    SELECT id INTO template_connections_id FROM public.templates WHERE admin_id = admin_id_var AND name = 'Holiday Homes Connections and Door';
    SELECT id INTO template_living_room_id FROM public.templates WHERE admin_id = admin_id_var AND name = 'Living Room';
    SELECT id INTO template_kitchen_id FROM public.templates WHERE admin_id = admin_id_var AND name = 'Kitchen';
    SELECT id INTO template_balcony_id FROM public.templates WHERE admin_id = admin_id_var AND name = 'Balcony';
    SELECT id INTO template_bedroom_id FROM public.templates WHERE admin_id = admin_id_var AND name = 'Bedroom';
    SELECT id INTO template_general_id FROM public.templates WHERE admin_id = admin_id_var AND name = 'Holiday Homes General and Cleaning Items';
    SELECT id INTO template_bathroom_id FROM public.templates WHERE admin_id = admin_id_var AND name = 'Bathroom';
