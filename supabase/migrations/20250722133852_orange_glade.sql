/*
  # Consolidated Database Schema for scopoStay

  This migration consolidates all previous migrations into a single comprehensive schema.
  It includes all tables, types, functions, triggers, RLS policies, and indexes needed for the application.

  ## Tables Created:
  1. **profiles** - User profile information
  2. **admin** - Company/admin records with subscription info
  3. **team_members** - Team member associations and roles
  4. **properties** - Property listings and details
  5. **template_categories** - Template organization categories
  6. **templates** - Inspection template definitions
  7. **template_items** - Individual items within templates
  8. **report_service_teams** - Teams for report distribution
  9. **property_checklists** - Property-specific inspection checklists
  10. **property_checklist_templates** - Template associations for checklists
  11. **inspections** - Inspection records and metadata
  12. **inspection_items** - Individual inspection item responses
  13. **stripe_customers** - Stripe customer records
  14. **stripe_subscriptions** - Stripe subscription data
  15. **stripe_orders** - Stripe order/payment records
...
CREATE POLICY "properties_manage_admins" ON properties FOR ALL TO authenticated 
USING (
    EXISTS (
...
CREATE POLICY "properties_select_members" ON properties FOR SELECT TO authenticated 
USING (
    EXISTS (
        SELECT 1 FROM admin a 
        WHERE a.id = properties.admin_id 
...
CREATE POLICY "template_categories_manage_admins" ON template_categories FOR ALL TO authenticated 
USING (
    EXISTS (
        SELECT 1 FROM admin a 
...
CREATE POLICY "template_categories_select_members" ON template_categories FOR SELECT TO authenticated 
USING (
    EXISTS (
        SELECT 1 FROM admin a 
        WHERE a.id = template_categories.admin_id 
...
CREATE POLICY "templates_manage_admins" ON templates FOR ALL TO authenticated 
USING (
    EXISTS (
        SELECT 1 FROM admin a 
...
CREATE POLICY "templates_select_members" ON templates FOR SELECT TO authenticated 
USING (
    EXISTS (
        SELECT 1 FROM admin a 
        WHERE a.id = templates.admin_id 
...
CREATE POLICY "template_items_manage_admins" ON template_items FOR ALL TO authenticated 
USING (
    EXISTS (
...
CREATE POLICY "template_items_select_members" ON template_items FOR SELECT TO authenticated 
USING (
    EXISTS (
        SELECT 1 FROM templates t 
        JOIN admin a ON t.admin_id = a.id 
...
CREATE POLICY "report_service_teams_manage_admins" ON report_service_teams FOR ALL TO authenticated 
USING (
    EXISTS (
        SELECT 1 FROM admin a 
...
CREATE POLICY "report_service_teams_select_members" ON report_service_teams FOR SELECT TO authenticated 
USING (
    EXISTS (
        SELECT 1 FROM admin a 
        WHERE a.id = report_service_teams.admin_id 
...
CREATE POLICY "property_checklists_manage_admins" ON property_checklists FOR ALL TO authenticated 
USING (
    EXISTS (
...
CREATE POLICY "property_checklists_select_members" ON property_checklists FOR SELECT TO authenticated 
USING (
    EXISTS (
        SELECT 1 FROM properties p 
        JOIN admin a ON p.admin_id = a.id 
...
-- Create RLS policies for property_checklist_templates (SIMPLIFIED)
CREATE POLICY "property_checklist_templates_access_for_members" ON property_checklist_templates FOR ALL TO authenticated 
USING (
    EXISTS (
...
CREATE POLICY "inspections_access_for_members" ON inspections FOR ALL TO authenticated 
USING (
    EXISTS (
        SELECT 1 FROM properties p 
        JOIN admin a ON p.admin_id = a.id 
...
CREATE POLICY "inspection_items_access_for_members" ON inspection_items FOR ALL TO authenticated 
USING (
    EXISTS (
...
CREATE TRIGGER update_report_service_teams_updated_at BEFORE UPDATE ON report_service_teams FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_property_checklists_updated_at BEFORE UPDATE ON property_checklists FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_property_checklist_templates_updated_at BEFORE UPDATE ON property_checklist_templates FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
