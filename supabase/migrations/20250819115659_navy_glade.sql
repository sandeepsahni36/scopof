/*
  # Fix create_demo_templates function - Missing template_bathroom_id variable

  1. Problem
    - The create_demo_templates function references template_bathroom_id variable without declaring it
    - This causes "column template_bathroom_id does not exist" error during user registration
    - New users cannot complete signup process

  2. Solution
    - Add template_bathroom_id uuid; to DECLARE section
    - Add INSERT statement to create Bathroom template and capture its ID
    - Ensure Bathroom template is created before its template_items are inserted

  3. Changes
    - Updated create_demo_templates function with proper variable declaration
    - Added Bathroom template creation with RETURNING id INTO template_bathroom_id
    - Maintained all existing template items for Bathroom template
*/
...
DECLARE
    ...
    template_bathroom_id uuid;  -- Added missing declaration
...
BEGIN
    ...
    INSERT INTO templates (id, admin_id, category_id, name, description, created_at, updated_at)
    VALUES (gen_random_uuid(), admin_id, null, 'Kitchen', 'Kitchen inspection template', now(), now())
    RETURNING id INTO template_kitchen_id;

    -- Create Bathroom template (was missing!)
    INSERT INTO templates (id, admin_id, category_id, name, description, created_at, updated_at)
    ...
    -- Insert Template Items for Bathroom (now template_bathroom_id is properly declared and initialized)
    INSERT INTO public.template_items (id, template_id, type, label, required, options, report_enabled, maintenance_email, "order") 
    VALUES
