...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'template_categories' AND policyname = 'Admins can manage their template categories') THEN
        CREATE POLICY "Admins can manage their template categories" ON template_categories FOR ALL TO authenticated USING (admin_id IN (SELECT admin.id FROM admin WHERE admin.owner_id = auth.uid())) WITH CHECK (admin_id IN (SELECT admin.id FROM admin WHERE admin.owner_id = auth.uid()));
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'template_categories' AND policyname = 'Team members can view template categories') THEN
        CREATE POLICY "Team members can view template categories" ON template_categories FOR SELECT TO authenticated USING (admin_id IN (SELECT tm.admin_id FROM team_members tm WHERE tm.profile_id = auth.uid()));
    END IF;
    ...
    -- Templates policies (CRITICAL FIX)
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'templates' AND policyname = 'Admins can manage their templates') THEN
        CREATE POLICY "Admins can manage their templates" ON templates FOR ALL TO authenticated USING (admin_id IN (SELECT admin.id FROM admin WHERE admin.owner_id = auth.uid())) WITH CHECK (admin_id IN (SELECT admin.id FROM admin WHERE admin.owner_id = auth.uid()));
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'templates' AND policyname = 'Team members can view templates') THEN
        CREATE POLICY "Team members can view templates" ON templates FOR SELECT TO authenticated USING (admin_id IN (SELECT tm.admin_id FROM team_members tm WHERE tm.profile_id = auth.uid()));
    END IF;
    ...
    -- Template items policies (CRITICAL FIX)
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'template_items' AND policyname = 'Admins can manage their template items') THEN
        CREATE POLICY "Admins can manage their template items" ON template_items FOR ALL TO authenticated USING (template_id IN (SELECT t.id FROM templates t JOIN admin a ON t.admin_id = a.id WHERE a.owner_id = auth.uid())) WITH CHECK (template_id IN (SELECT t.id FROM templates t JOIN admin a ON t.admin_id = a.id WHERE a.owner_id = auth.uid()));
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'template_items' AND policyname = 'Team members can view template items') THEN
        CREATE POLICY "Team members can view template items" ON template_items FOR SELECT TO authenticated USING (template_id IN (SELECT t.id FROM templates t JOIN team_members tm ON t.admin_id = tm.admin_id WHERE tm.profile_id = auth.uid()));
    END IF;
